...
export async function GET(request: NextRequest) {
  ...
  try {
    const emails = listOutreachEmails(db, user.id, status as OutreachEmailStatus | undefined);
    return jsonData({ emails });
  } catch (e) {
    return jsonError(e instanceof Error ? e.message : "Failed to list outreach emails", 500);
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const contact = getContact(db, user.id, parsed.data.contactId);
    ...
    const email = createOutreachEmail(db, user.id, {
    ...
    return jsonError(e instanceof Error ? e.message : "Failed to generate draft", 500);
  }
}
